...
class _HealtyFoodViewState extends State<HealtyFoodView> {
        ...
        Expanded(
          child: ListView.builder(
            itemBuilder: (context, index) => Dismissible(
              ...
              child: Card(
                child: Container(height: 100, child: Center(child: Text(index.toString()))),
              ),
            ),
          ),
        )
  ...
  Text categoryTitle() => Text("Categoires", style: Theme.of(context).textTheme.headline6);
  ...
  Color buildCategoryButtonColor(int index) =>
      count == index ? Theme.of(context).colorScheme.onError : Theme.of(context).colorScheme.primary;
  ...
  IconButton iconButton() => IconButton(icon: Icon(Icons.search, color: Colors.black), onPressed: () {});

  Text text() => Text("Hello", style: Theme.of(context).textTheme.headline6);

  BottomNavigationBar bars() {
    return BottomNavigationBar(items: [
      BottomNavigationBarItem(icon: Icon(Icons.accessibility), title: Text("data")),
      BottomNavigationBarItem(icon: Icon(Icons.accessibility), title: Text("data")),
    ]);
  }
}
